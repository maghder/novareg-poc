  ...
  try {
    const bruts = await directus.request(
      readItems('bo_courriers', {
        filter: { sens: { _eq: 'depart' }, statut_code: { _eq: 'enregistre' } },
        fields: [
          'id',
          'numero_chrono',
          'objet',
    ...
    courriers.value = bruts.map((c) => ({
      ...
      destinataire: c.parties?.find((p) => p.role_code === 'destinataire')?.libelle_snapshot ?? null
    }))
  } catch (e) {
    error.value =
      "Impossible de charger la file d'expédition. Vérifiez les droits de lecture sur " +
...
<template>
  <section>
    <div class="toolbar">
      <h1>Courriers à expédier</h1>
      <span class="count" v-if="!loading">{{ courriers.length }} en attente</span>
    </div>
    <p class="hint">Courriers départ enregistrés, prêts à être préparés pour l'envoi.</p>

    <p v-if="loading" class="hint">Chargement…</p>
    <p v-else-if="error" class="error">{{ error }}</p>
    <p v-else-if="!courriers.length" class="hint">Aucun courrier en attente d'expédition.</p>
    ...
    <table v-else class="table">
      ...
      <tbody>
        <tr
          ...
          :key="c.id"
          class="row-link"
          @click="$router.push(`/expedition/${c.id}`)"
        >
          <td class="mono">{{ c.numero_chrono }}</td>
          <td class="objet">{{ c.objet }}</td>
          <td class="muted">{{ c.destinataire || '—' }}</td>
